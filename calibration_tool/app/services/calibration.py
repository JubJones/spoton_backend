...
class CalibrationManager:
    ...
    def add_destination_point(self, camera_id: str, point: Point) -> int:
        session = self.get_session(camera_id)
        if len(session.destination_points) >= len(session.source_points):
            raise ValueError("Destination point count cannot exceed source point count")
    ...
    def undo_last_point(self, camera_id: str) -> None:
        session = self.get_session(camera_id)
        if session.destination_points and len(session.destination_points) == len(session.source_points):
            session.destination_points.pop()
    ...
    def can_compute_homography(self, camera_id: str) -> bool:
        session = self.get_session(camera_id)
        return len(session.source_points) >= 4 and len(session.source_points) == len(session.destination_points)
