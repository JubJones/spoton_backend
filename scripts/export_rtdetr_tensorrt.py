#!/usr/bin/env python3
"""
TensorRT Export Script for RT-DETR-L Model
...
Usage:
    python scripts/export_rtdetr_tensorrt.py

The script will:
1. Load the PyTorch model (rtdetr-l.pt)
...
def main():
    ...
    print("\n" + "="*60)
    print("RT-DETR TensorRT Export Tool")
    print("="*60 + "\n")
    ...
    pt_model = weights_dir / "rtdetr-l.pt"
    ...
    if not pt_model.exists():
        ...
        pt_files = list(weights_dir.glob("rtdetr*.pt"))
        if pt_files:
            pt_model = pt_files[0]
            print(f"\n📌 Using found model: {pt_model.name}")
        else:
            print(f"\nNo {pt_model.name} found. Please add your model to the weights/ directory.")
            sys.exit(1)
    ...
    print(f"   Before: weights/rtdetr-l.pt")
