...
@pytest.mark.asyncio
async def test_on_startup_device_selection_fails(
    mock_fastapi_app: MagicMock,
    mock_settings, # MODIFIED: Added mock_settings
    mock_get_selected_device_util: MagicMock,
    mock_detector_class_in_events, # MODIFIED: Added fixture
    mock_tracker_factory_class_in_events, # MODIFIED: Added fixture
    mock_homography_service_class_in_events, # MODIFIED: Added fixture
    mocker
):
    """Tests on_startup when device selection utility raises an error."""
    # MODIFIED: Patch settings for the event_handlers module
    mocker.patch("app.core.event_handlers.settings", mock_settings)
    ...
    # MODIFIED: Removed direct mocker.patch calls for classes, as fixtures handle this now.
    # The fixtures mock_detector_class_in_events, mock_tracker_factory_class_in_events,
    # and mock_homography_service_class_in_events will be used automatically.

    await event_handlers.on_startup(mock_fastapi_app)

    # MODIFIED: Use assert_any_call for logger checks
    mock_logger_error.assert_any_call("Failed to determine compute device during startup: Device detection crashed", exc_info=True)
...
@pytest.mark.asyncio
async def test_on_startup_detector_preload_fails(
    mock_fastapi_app: MagicMock,
    mock_settings, # MODIFIED: Added mock_settings
    mock_detector_class_in_events,
    mock_tracker_factory_class_in_events, # MODIFIED: Added fixture
    ...
    # MODIFIED: Patch settings and ensure device selection part works
    mocker.patch("app.core.event_handlers.settings", mock_settings)
    ...
    # MODIFIED: Removed direct mocker.patch calls for CameraTrackerFactory and HomographyService

    await event_handlers.on_startup(mock_fastapi_app)

    # MODIFIED: Use assert_any_call for logger checks
    mock_logger_error.assert_any_call("CRITICAL: Failed to preload detector model: Detector model load error", exc_info=True)
