...
class ReIDStateManager:
    ...
    def __init__(self, task_id: uuid.UUID):
        ...
        self.global_id_last_seen_frame: Dict[GlobalID, int] = {} # Stores processed_frame_count
        self.track_last_reid_frame: Dict[TrackKey, int] = {} # Stores processed_frame_count
    ...
    def _normalize_embedding(self, embedding: Optional[FeatureVector]) -> Optional[FeatureVector]:
        ...
        # Ensure embedding is a NumPy array first
        if isinstance(embedding, list): embedding_np = np.array(embedding, dtype=np.float32)
        elif isinstance(embedding, np.ndarray): embedding_np = embedding.astype(np.float32)
        else:
            logger.warning(f"Invalid type for embedding: {type(embedding)}. Cannot normalize.")
            return None
        
        if embedding_np.size == 0: # Handle empty array case
            return FeatureVector(np.array([]))
    ...
    def _calculate_similarity_matrix(
        ...
        if query_embeddings.size == 0 or gallery_embeddings.size == 0:
            # logger.debug(f"[Task {self.task_id}] Empty query or gallery for similarity calculation.")
            return None
        try:
    ...
    def _apply_handoff_filter_for_match(
        ...
        if not trigger_info: # No active trigger FOR THIS TRACK
            # Default SpotOn logic: GID must be plausible for current camera based on general overlap
        ...
        # Active trigger for this track_key exists
    ...
    def _update_gallery_with_ema(
        ...
        if new_embedding_norm is None: return # Cannot update with invalid new embedding
    ...
    async def associate_features_and_update_state(
        ...
        for i, tk in enumerate(query_track_keys_list):
            if tk not in current_assignments or current_assignments[tk][0] is None:
                ...
                norm_embedding = self._normalize_embedding(query_embeddings_np[i])
                if norm_embedding is not None: self.reid_gallery[new_gid] = norm_embedding
        ...
        for cam_id, gid_map in assignments_by_cam_gid.items():
            for gid, track_score_list in gid_map.items():
                if len(track_score_list) > 1: 
                    ...
                    # logger.warning(f"[Task {self.task_id}][{cam_id}] Conflict for GID {gid}. Tracks: {[(ts[0][1], f'{ts[1]:.2f}') for ts in track_score_list]}. Keeping T:{winner_tk[1]}.")
        ...
        if reverted_keys_for_second_pass:
            ...
            for tk_reverted in reverted_keys_for_second_pass:
                ...
                if current_main_gallery_embeds_np.size > 0:
                    ...
                    if sim_matrix_2pass is not None and sim_matrix_2pass.size > 0:
                        ...
                        if max_sim_2pass >= settings.REID_SIMILARITY_THRESHOLD:
                            ...
                            if original_conflict_info:
                                winner_of_conflict_tk = original_conflict_info[0][0] 
                            ...
                            if can_assign_gid_2pass and self._apply_handoff_filter_for_match(tk_reverted, gid_2pass, active_triggers_map):
                                ...
                                self._update_gallery_with_ema(gid_2pass, reverted_embedding, tk_reverted, "main") 
                ...
                if not assigned_in_second_pass:
                    ...
                    norm_reverted_embedding = self._normalize_embedding(reverted_embedding)
                    if norm_reverted_embedding is not None: self.reid_gallery[new_gid_for_reverted] = norm_reverted_embedding
