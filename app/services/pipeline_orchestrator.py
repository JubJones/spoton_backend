...
from app.api.v1.schemas import MediaURLEntry # Import for media notification
...
class PipelineOrchestratorService:
    ...
    async def update_task_status(self, task_id: uuid.UUID, status: Optional[str] = None,
                                 ...
                                 details: Optional[str] = None, increment_processed_batches: Optional[int] = None):
        ...
        if increment_processed_batches:
    ...
    async def run_processing_pipeline(self, task_id: uuid.UUID, environment_id: str):
        ...
        try:
            ...
            global_batch_idx_for_task = 0

            for sub_video_batch_idx in range(max_sub_video_batches): # Renamed for clarity
                ...
                local_video_paths_map = await self.video_data_manager.download_sub_videos_for_environment_batch(
                    task_id, environment_id, sub_video_batch_idx # Pass 0-indexed
                )
                ...
                # --- Notify frontend about available media ---
                media_url_entries: List[MediaURLEntry] = []
                for cam_id, local_path in local_video_paths_map.items():
                    # Construct the URL that points to the backend's media serving endpoint
                    # Base URL might need to be configurable or derived from request in a real scenario
                    # For now, assuming localhost and default port, and known API prefix
                    # Example: /api/v1/media/tasks/{task_id}/environments/{env_id}/cameras/{cam_id}/sub_videos/{filename}
                    ...
                    media_url_entries.append(
                        MediaURLEntry(camera_id=str(cam_id), sub_video_filename=video_filename, url=media_url)
                    )

                if media_url_entries:
                    await self.notification_service.send_media_available_notification(
                        str(task_id),
                        sub_video_batch_idx, # Send the 0-indexed batch number
                        media_url_entries
                    )
                ...
                frames_in_current_sub_video_batch_count = 0
                ...
                while True:
                    ...
                    processed_data_batch = await self.multi_camera_processor.process_frame_batch(
                        task_id, environment_id, reid_manager, frame_batch_dict, global_batch_idx_for_task
                    )

                    ws_output_frame_data = {
                        "frame_index": global_batch_idx_for_task -1,
